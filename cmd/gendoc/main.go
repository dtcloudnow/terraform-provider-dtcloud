...
//
// It is deliberately the second half of a two-step pipeline:
//
//	Go schema + examples  --tfplugindocs-->  docs/  --gendoc-->  docusaurus
//
// docs/ is the canonical, Registry-format output; nothing here knows anything
// about the provider. That keeps one source of truth: a page's subcategory, its
// text and its examples are decided upstream, and this command only re-frames
// what it finds. Adding a resource therefore needs no change in this file.
//
// Usage:
//
...
// subcategoryOrder is the order the Registry subcategories are listed in. A
// subcategory that appears in docs/ but not here sorts last, alphabetically, so
// a new one shows up rather than disappearing.
...
// schema heading into Docusaurus explicit heading ids.
//
// tfplugindocs emits an empty `<a id="...">` because the Registry renders plain
// markdown. That tag does render here, but Docusaurus collects anchors from
// headings only, so every "see below for nested schema" link is reported broken
// and none of them reaches the table of contents. Moving the id onto the
// heading itself keeps the same link targets and makes both work.
...
// mdxSafe escapes the characters MDX reads as JSX or as an expression. Fenced
// code blocks and inline code spans are left exactly as they are -- a `<` or `{`
// inside them is already inert, and escaping it would corrupt the example --
// and so are the anchor tags tfplugindocs emits for nested schema links, which
// are valid JSX and are what the "see below" links point at.
